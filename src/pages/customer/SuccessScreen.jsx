...
export default function SuccessScreen() {
  ...
  const { bookings, connectionReady } = useApp();
  ...
  const liveBooking = bookings.find((b) => b.id === bookingId);
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        {accepted ? (
          <p className="text-muted mb-2">
            <span className="text-[#10B981] font-semibold">{handymanName}</span> accepted your request — they're on the way
          </p>
        ) : (
          <p className="text-muted mb-2 flex items-center gap-1">
            <Loader2 className="w-3.5 h-3.5 animate-spin text-accent" />
            Waiting for a KWIKFIXER to accept your request{connectionReady ? ' (live updates enabled)' : ''}
          </p>
        )}

        <div className={`mt-3 px-4 py-2 rounded-full text-xs font-semibold flex items-center gap-1 ${accepted ? 'bg-[#10B981]/15 text-[#10B981] border border-[#10B981]/30' : 'bg-accent/10 text-accent border border-accent/30'}`}>
          {accepted ? <CheckCircle className="w-3.5 h-3.5" /> : <Wrench className="w-3.5 h-3.5 animate-pulse" />}
          {accepted ? `Accepted by ${handymanName}` : 'Status: Waiting for acceptance'}
        </div>

        <div className="bg-navy-800 border border-white/10 rounded-2xl shadow-lg p-6 w-full max-w-sm mb-4 mt-6">
        ...
        <div className="bg-accent/10 border border-accent/30 rounded-2xl p-4 w-full max-w-sm mb-6">
        ...
        {accepted && (
          <button
            onClick={() => navigate(`/customer/active-job/${bookingId}`)}
            className="w-full max-w-sm bg-accent text-white py-3 rounded-2xl font-bold mb-3 hover:bg-accent-dark"
          >
            Track Active Job
          </button>
        )}
